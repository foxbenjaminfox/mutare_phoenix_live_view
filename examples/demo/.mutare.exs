...
# base `mutare_phoenix` / `mutare_plug` ones it depends on) are already on the code path and
# the lines below are unnecessary. Here the demo lives *inside* the `mutare_phoenix_live_view`
# repo, where the packages are the root project and its deps — which the `mutare` task does
# not add to the demo's code path — so we append the compiled package `ebin`s ourselves (run
# `mix compile` in the repo root first).
for pkg <- ["mutare_plug", "mutare_phoenix", "mutare_phoenix_live_view"],
...
# base `mutare_plug` / `mutare_phoenix` families it composes with, plus the built-in
# `:convention`, so the
...
[
  paths: ["lib/demo"],
  mutators:
    [:convention] ++ Mutare.Plug.all() ++ Mutare.Phoenix.all() ++ Mutare.Phoenix.LiveView.all(),
  extensions: [Mutare.Phoenix]
]
