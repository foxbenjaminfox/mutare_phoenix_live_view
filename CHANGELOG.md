...
- `Mutare.Phoenix.LiveView.Event` (`:lv_event`) — removes `push_event/3`,
  collapsing to the socket (or `Function.identity()` when piped).
- `Mutare.Phoenix.LiveView.SendUpdate` (`:lv_send_update`) — removes
  `send_update/2,3` / `send_update_after/3,4`, collapsing to each call's
  faithful return (`:ok` / a fresh `make_ref()`).
